...
const fetchBranchs = async (req, res) => {
    try {
        ...
        if (!branches || branches.length === 0) {
            return res.status(404).json({ success: true, message: "No branches found" });
        }
...
const createBranch = async (req, res) => {
    const {name, location, contactNumber, openingHours, privateCode } = req?.body;
    if (!name || !location || !contactNumber || !openingHours) {
        return res.status(400).json({success: false, message: "Missing important information"});
    }
...
export { fetchBranchs, fetchBranchById, createBranch, updateBranch, deleteBranch };